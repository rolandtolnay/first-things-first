...
Development uses local Supabase only; production is the only hosted Supabase project. Do not create, relink to, or depend on a hosted dev project for normal work. Bootstrap local dev with `npm run dev:setup`, which starts Supabase, applies migrations, loads `supabase/seed.sql`, and writes local public env values. The deterministic local login user is `dev@example.com`; magic-link emails land in local Inbucket. Prod operations must be explicit: apply migrations with `npm run db:push:prod`, which resets local Supabase as the migration rehearsal, runs tests/lint, prompts for the prod DB password, dry-runs prod, asks for confirmation, then pushes the same checked-in migrations. Direct DB connections may fail on IPv4-only networks with `no route to host`; use the dashboard **Session pooler** string instead. Do not guess pooler hosts — this prod project worked via `aws-1-eu-central-1.pooler.supabase.com`, while `aws-0` produced `ENOTFOUND tenant/user ... not found`.
