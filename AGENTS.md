...
A role exists in two places: the durable `public.roles` table (`store.activeRoles`) and a per-week **Role Snapshot** inside `weeks.data->'roles'`. The sidebar renders and edits **snapshots** (`RoleList` reads `currentWeek.roles`), but role mutations also write the durable table keyed on `snapshot.id`. There is **no backfill**: the durable table is only populated by the "Add role" flow, so any week created before the `roles` table existed (or any user who never used Add role) has snapshots whose ids have **no durable row** — verified on prod, where `roles` was empty while weeks held 11 snapshots.

Invariant to preserve: a durable write for a snapshot id may hit **zero rows**. Never use `UPDATE … .eq("id", id).single()` for role mutations — PostgREST returns a 0-row **406** (`PGRST116`), surfaced as a plain object (not an `Error`), so `roleErrorMessage` shows the literal "Failed to save role changes." Both `updateRoleDefaults` and `archiveRole` are now **upserts on `id`** that materialize the missing row; the store passes the full snapshot (name/color/order) so an insert has every NOT NULL column. `updateRole` also tolerates the active-name `23505` collision; archive can't hit it (an archived row is outside the active-name unique index). Any future durable role write reached from a snapshot id (e.g. a new mutation) must follow the same materialize-on-demand pattern, not `.single()`.
